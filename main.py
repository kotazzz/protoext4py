...
def create_block_groups(f, num_groups: int):
    ...
    for group_num in range(num_groups):
        ...
        inode_table_blocks = (
            INODES_PER_GROUP + inodes_per_block - 1
        ) // inodes_per_block
        ...
        group_desc = GroupDesc(
            ...
            - (1 if group_num == 0 else 0),  # Reserve root inode in group 0
        )
        ...
        print(
            f"Group {group_num}: bitmap_block={block_bitmap_block}, inode_bitmap={inode_bitmap_block}, inode_table={inode_table_block}"
        )
...
def create_root_inode(f):
    ...
    dot_entry = (
        struct.pack("<III", 2, 16, 1) + b"\x02\x00" + b"." + b"\x00" * 1
    )  # 16 bytes total

    # Create .. entry (points to itself for root - inode 2)
    dotdot_entry = (
        struct.pack("<III", 2, 16, 2) + b"\x02\x00" + b".." + b"\x00" * 0
    )  # 16 bytes total

    # Write directory entries
    dir_data = (
        dot_entry + dotdot_entry + b"\x00" * (BLOCK_SIZE - 32)
