...
BLOCK_SIZE = 4096
INODE_SIZE = 96  # Updated to match actual Inode structure size (rounded up to multiple of 4)
...
class DirEntry:
    ...
    def pack(self) -> bytes:
        ...
        entry_len = 14 + len(name_bytes)  # 12 bytes header + 1 file_type + 1 reserved + name
        # Align to 4 bytes
    ...
    def unpack(cls, data: bytes, offset: int = 0) -> Tuple['DirEntry', int]:
        ...
        inode_num, entry_len, name_len = struct.unpack('<III', data[offset:offset+12])
...
class FileSystem:
                    ...
                    if bitmap[byte_idx] != 0xFF:  # Not all bits set
                        for bit_idx in range(8):
                            if not (bitmap[byte_idx] & (1 << bit_idx)):
                                ...
                                allocated_block = group_num * BLOCKS_PER_GROUP + byte_idx * 8 + bit_idx
    ...
    def _add_directory_entry(self, dir_inode_num: int, filename: str, file_inode_num: int, file_type: int = 0):
        ...
        for extent in dir_inode.extents:
            ...
            for block_idx in range(extent.block_count):
                ...
                if block_idx == extent.block_count - 1 and block_used + len(entry_data) <= BLOCK_SIZE:
    ...
    def open(self, path: str, flags: int = O_RDONLY) -> int:
        ...
        try:
            ...
            if flags & O_CREAT:
                ...
                self._add_directory_entry(parent_inode_num, filename, inode_num, 1)  # Regular file type
            else:
                raise
    ...
    def read(self, fd: int, size: int, offset: Optional[int] = None) -> bytes:
        ...
        for extent in inode.extents:
            ...
            if current_offset < extent_start_byte + extent_size_bytes:
                ...
                bytes_to_read = min(actual_size - bytes_read, extent_size_bytes - extent_offset)
                
                # Read blocks
                start_block = extent.start_block + (extent_offset // BLOCK_SIZE)
                block_offset = extent_offset % BLOCK_SIZE
                
                while bytes_to_read > 0 and start_block < extent.start_block + extent.block_count:
    ...
    def write(self, fd: int, data: bytes, offset: Optional[int] = None) -> int:
        ...
        for extent in inode.extents:
            ...
            if current_offset < extent_size_bytes:
                ...
                bytes_to_write = min(len(data) - bytes_written, extent_size_bytes - extent_offset)
                ...
                while bytes_to_write > 0 and start_block < extent.start_block + extent.block_count:
                    ...
                    chunk_data = data[data_offset:data_offset + (end_offset - block_offset)]
    ...
    def mkdir(self, path: str, mode: int = 0o755):
        ...
        # Add to parent directory
        self._add_directory_entry(parent_inode_num, dirname, dir_inode_num, 2)  # Directory type
